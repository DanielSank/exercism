...
#[test]
fn test_best_paired_hand() {
    let cards = Card::from_str("AS AH AD AC KS");
    assert_eq!(poker::best_paired_hand(&cards), Hand::FourOfAKind(14, 13));
    let cards = Card::from_str("3H 3S 3D 5D QC");
    assert_eq!(poker::best_paired_hand(&cards), Hand::ThreeOfAKind(3, 12, 5));
    let cards = Card::from_str("3H 3S 5D 5D QC");
    assert_eq!(poker::best_paired_hand(&cards), Hand::TwoPair(5, 3, 12));
    let cards = Card::from_str("3H 3S 3D 8D 8C");
    assert_eq!(poker::best_paired_hand(&cards), Hand::FullHouse(3, 8));
    let cards = Card::from_str("3H 4S 5D 6D 8C");
    assert_eq!(poker::best_paired_hand(&cards), Hand::HighCard(8, 6, 5, 4, 3));
}

#[test]
fn test_str_to_card() {
    assert_eq!(Card::new("10H"), Card{rank: 10, suit: Suit::Heart})
}

#[test]
fn test_hand_order() {
    let straight_flush = Hand::StraightFlush(6);
    let four_of_a_kind = Hand::FourOfAKind(10, 13);
    let full_house = Hand::FullHouse(3, 9);
    let flush = Hand::Flush(11, 9, 5, 4, 3);
    let straight = Hand::Straight(10);
    let three_of_a_kind = Hand::ThreeOfAKind(5, 10, 4);
    let two_pair = Hand::TwoPair(9, 3, 14);
    let pair = Hand::OnePair(3, 12, 7, 3);
    let high = Hand::HighCard(10, 6, 5, 4, 3);
